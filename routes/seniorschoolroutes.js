...
const uploadToCloudinary = (buffer) => {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'image', folder: 'SeniorSchool' },
      (error, result) => {
...
router.post('/upload', upload.array('image', 10), async (req, res) => {
  try {
    ...
    const existingContent = await Content.findOne({ section: 'Senior School' });
    ...
    if (existingContent) {
      ...
      return res.status(200).json({ message: 'Senior School content updated', content: updatedContent });
    } else {
      ...
      return res.status(201).json({ message: 'Senior School content created', content: savedContent });
    }
  } catch (err) {
